...
export class ListeningComponent implements OnInit {
  ...
  loadListeningList() {
    ...
    this.http
      .get<ApiResponse<ListResponse>>(
        `${environment.apiUrl}/listenings?limit=${this.pageSize}&page=${this.pageIndex}`
      )
      .subscribe({
        next: (response) => {
          if (response.status) {
  ...
  handleCreateOk() {
    if (this.createForm.valid) {
      ...
      this.http
        .post<ApiResponse<any>>(`${environment.apiUrl}/listening`, formData)
        .subscribe({
          next: (response) => {
            if (response.status) {
  ...
  viewDetail(id: number) {
    this.http
      .get<ApiResponse<ListeningDetail>>(
        `${environment.apiUrl}/listening/${id}`
      )
      .subscribe({
        next: (response) => {
          if (response.status) {
